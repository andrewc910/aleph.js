...
import type { APIHandler, RouterURL } from "./types.ts";
import util, {
  hashShort,
  MB,
  reHashJs,
...
export class Project {
  ...
  get srcDir() {
    return path.join(this.config.appRoot, this.config.srcDir);
  }
  ...
  async callAPI(
    ...
    const [url] = this.#apiRouting.createRouter(
      { ...loc, pathname: decodeURI(loc.pathname) },
    );
    if (url.pagePath != "") {
      ...
      if (this.#modules.has(moduleID)) {
        try {
          const { default: handle } = await import(
            "file://" + this.#modules.get(moduleID)!.jsFile
          );
          if (util.isFunction(handle)) {
            ...
            req.respond({
              ...
              body: JSON.stringify(
                { error: { status: 404, message: "handle not found" } },
              ),
  ...
  async getPageHtml(
    ...
    if (!this.isSSRable(loc.pathname)) {
      ...
      return [
        url.pagePath === "" ? 404 : 200,
        await this.getSPAIndexHtml(),
        null,
      ];
    }
    ...
    const { url, status, head, scripts, body, data } = await this._renderPage(
      loc,
    );
    const html = createHtml({
      ...
      scripts: [
        ...
        {
          src: util.cleanPath(
            `${baseUrl}/_aleph/main.${mainModule.hash.slice(0, hashShort)}.js`,
          ),
          type: "module",
        },
  ...
  async getSPAIndexHtml() {
    ...
    const html = createHtml({
      ...
      scripts: [
        ...
        {
          src: util.cleanPath(
            `${baseUrl}/_aleph/-/deno.land/x/aleph/nomodule.js${
              this.isDev ? "?dev" : ""
            }`,
          ),
          nomodule: true,
  ...
  async build() {
    ...
    if (ssr) {
      ...
      await Promise.all(
        Array.from(paths).map(async (pathname) => {
          if (this.isSSRable(pathname)) {
            ...
            if (status == 200) {
              ...
              log.info(
                "  ○",
                pathname,
                colors.dim("• " + util.bytesString(html.length)),
              );
            } else if (status == 404) {
              log.info(
                "  ○",
                colors.dim(pathname),
                colors.red(`Page not found`),
              );
            } else if (status == 500) {
      ...
      const fbHtmlFile = path.join(
        outputDir,
        util.isPlainObject(ssr) && ssr.fallback
          ? ssr.fallback
          : "_fallback.html",
      );
    ...
    if (existsDirSync(publicDir)) {
      ...
      for await (
        const { path: p } of walk(
          publicDir,
          { includeDirs: false, skip: [/\.DS_Store$/] },
        )
      ) {
    ...
    await Promise.all(
      Array.from(outputModules).map((moduleID) => {
        const {
          sourceFilePath,
          loader,
          isRemote,
        ...
        const jsFile = path.join(
          saveDir,
          name + (isRemote ? "" : "." + hash.slice(0, hashShort)),
        ) + ".js";
        if (isRemote) {
        ...
        return Promise.all([
          ...
          sourceMap && jsSourceMap
            ? ensureTextFile(jsFile + ".map", jsSourceMap)
            : Promise.resolve(),
        ]);
      }),
    );
    ...
    log.info(
      "  {}",
      colors.bold(deps.count.toString()),
      "deps",
      colors.dim(`• ${util.bytesString(deps.bytes)} (mini, uncompress)`),
    );
    log.info(
      "  {}",
      colors.bold(modules.count.toString()),
      "modules",
      colors.dim(`• ${util.bytesString(modules.bytes)} (mini, uncompress)`),
    );
    log.info(
      "  {}",
      colors.bold(styles.count.toString()),
      "styles",
      colors.dim(`• ${util.bytesString(styles.bytes)} (mini, uncompress)`),
    );
    ...
    for await (const event of w) {
      for (const p of event.paths) {
        ...
        const validated = (() => {
          ...
          if (
            path.startsWith("/.aleph/") ||
            path.startsWith(this.config.outputDir)
          ) {
            return false;
          }
          ...
          if (
            ...
            (reModuleExt.test(path) || reMDExt.test(path))
          ) {
            return true;
          }
          let isDep = false;
                ...
                if (hmrable) {
                  if (type === "add") {
                    this.#fsWatchListeners.forEach((e) =>
                      e.emit("add", mod.id, mod.hash)
                    );
                  } else {
                    this.#fsWatchListeners.forEach((e) =>
                      e.emit("modify-" + mod.id, mod.hash)
                    );
                  }
                }
                if (moduleID === "/app.js") {
  ...
  private _getRouteModule({ id, hash }: Module): RouteModule {
    const deps = this._lookupDeps(id).filter(({ isData, isStyle }) =>
      !!isData || !!isStyle
  ...
  private _moduleFromURL(url: string): Module {
    const isRemote = reHttp.test(url) ||
      (url in this.config.importMap.imports &&
        reHttp.test(this.config.importMap.imports[url]));
    const sourceFilePath = fixImportUrl(url);
    const id =
      (isRemote
        ? "//" + util.trimPrefix(sourceFilePath, "/-/")
        : sourceFilePath).replace(reModuleExt, ".js");
    let loader = "";
  ...
  private async _createMainModule(): Promise<Module> {
    ...
    const config: Record<string, any> = {
      ...
      preloadModules: ["/404.js", "/app.js"].filter((id) =>
        this.#modules.has(id)
      ).map((id) => {
    ...
    module.jsContent = [
      ...
      `bootstrap(${
        JSON.stringify(config, undefined, this.isDev ? 4 : undefined)
      })`,
    ].filter(Boolean).join(this.isDev ? "\n" : ";");
    module.hash = getHash(module.jsContent);
    module.jsFile = path.join(
      this.buildDir,
      `main.${module.hash.slice(0, hashShort)}.js`,
    );
    module.deps = [
      ...
      url: String(url),
      hash: this.#modules.get(
        String(url).replace(reHttp, "//").replace(reModuleExt, ".js"),
      )?.hash || "",
    }));
  ...
  private async _compile(
    ...
    if (existsFileSync(metaFile)) {
      const { sourceHash, hash, deps } = JSON.parse(
        await Deno.readTextFile(metaFile),
      );
      const jsFile = path.join(
        saveDir,
        name + (mod.isRemote ? "" : "." + hash.slice(0, hashShort)),
      ) + ".js";
      if (
        util.isNEString(sourceHash) && util.isNEString(hash) &&
        util.isArray(deps) && existsFileSync(jsFile)
      ) {
        try {
    ...
    if (mod.isRemote) {
      let dlUrl = url;
      const { imports } = this.config.importMap;
      for (const importPath in imports) {
        ...
        if (importPath === url) {
          ...
          dlUrl = util.trimSuffix(alias, "/") + "/" +
            util.trimPrefix(url, importPath);
          break;
        }
      }
      if (
      ...
      if (
        /^(https?:\/\/[0-9a-z\.\-]+)?\/react\-dom(@[0-9a-z\.\-]+)?(\/server)?\/?$/i
          .test(dlUrl)
      ) {
      ...
      if (dlUrl.startsWith("https://esm.sh/")) {
        ...
        dlUrl = `https://deno.land/x/aleph@v${version}/` +
          util.trimPrefix(dlUrl, "https://deno.land/x/aleph/");
      }
      if (mod.sourceHash === "") {
        ...
        try {
          ...
          if (resp.status != 200) {
            throw new Error(
              `Download ${url}: ${resp.status} - ${resp.statusText}`,
            );
          }
          sourceContent = await Deno.readAll(
            fromStreamReader(resp.body!.getReader()),
          );
    ...
    if (shouldCompile) {
      ...
      for (const plugin of this.config.plugins) {
        if (plugin.test.test(url) && plugin.transform) {
          const { code, loader = "js" } = await plugin.transform(
            sourceContent,
            url,
          );
          sourceCode = code;
      ...
      if (mod.loader === "css") {
        ...
        mod.jsContent = [
          ...
          `  }, []);`,
          `  return React.createElement("div", {className: "markdown-page", ref, dangerouslySetInnerHTML: {__html: ${
            JSON.stringify(html)
          }}});`,
          `}`,
          `MarkdownPage.meta = ${
            JSON.stringify(props, undefined, this.isDev ? 4 : undefined)
          };`,
          this.isDev && `_s(MarkdownPage, "useRef{ref}\\nuseEffect{}");`,
          this.isDev && `$RefreshReg$(MarkdownPage, "MarkdownPage");`,
        ].filter(Boolean).map((l) => !this.isDev ? String(l).trim() : l).join(
          this.isDev ? "\n" : "",
        );
        mod.jsSourceMap = "";
        mod.hash = getHash(mod.jsContent);
      } else if (
        mod.loader === "js" || mod.loader === "ts" || mod.loader === "jsx" ||
        mod.loader === "tsx"
      ) {
        ...
        const compileOptions = {
          ...
          rewriteImportPath: (path: string) =>
            this._resolveImportURL(mod, path),
          signUseDeno: (id: string) => {
            const sig = "useDeno." +
              (new Sha1()).update(id).update(version).update(
                Date.now().toString(),
              ).hex().slice(0, hashShort);
            useDenos.push(sig);
            return sig;
          },
        };
        const { diagnostics, outputText, sourceMapText } = compile(
          mod.sourceFilePath,
          sourceCode,
          compileOptions,
        );
        ...
        const jsContent = outputText.replace(
          /import\s*{([^}]+)}\s*from\s*("|')tslib("|');?/g,
          "import {$1} from " + JSON.stringify(getRelativePath(
            path.dirname(mod.sourceFilePath),
    ...
    // compile deps
    for (
      const dep of mod.deps.filter(({ url, external }) =>
        !url.startsWith("#useDeno.") && !external
      )
    ) {
      ...
      if (dep.hash !== depMod.hash) {
        ...
        if (!reHttp.test(dep.url)) {
          ...
          mod.jsContent = mod.jsContent.replace(
            /(import|Import|export)([\s\S]*?)(from\s*:?\s*|\(|)("|')([^'"]+)("|')(\)|;)?/g,
            (s, key, fields, from, ql, importPath, qr, end) => {
              if (
                reHashJs.test(importPath) &&
                importPath.slice(0, importPath.length - (hashShort + 4)) ===
                  depImportPath
              ) {
                return `${key}${fields}${from}${ql}${depImportPath}.${
                  dep.hash.slice(0, hashShort)
                }.js${qr}${end || ""}`;
              }
              return s;
            },
    ...
    if (fsync) {
      mod.jsFile = path.join(
        saveDir,
        name + (mod.isRemote ? "" : `.${mod.hash.slice(0, hashShort)}`),
      ) + ".js";
      ...
      await Promise.all([
        ...
        mod.jsSourceMap !== ""
          ? ensureTextFile(mod.jsFile + ".map", mod.jsSourceMap)
          : Promise.resolve(),
      ]);
    }
  ...
  private _updateDependency(
    ...
    this.#modules.forEach((mod) => {
      mod.deps.forEach((dep) => {
        if (
          dep.url === depPath && dep.hash !== depHash && !tracing?.has(mod.id)
        ) {
          const depImportPath = getRelativePath(
          ...
          if (mod.id === "/main.js") {
            ...
            mod.jsContent = mod.jsContent.replace(
              /(import|export)([^'"]*)("|')([^'"]+)("|')(\)|;)?/g,
              (s, key, from, ql, importPath, qr, end) => {
                if (
                  reHashJs.test(importPath) &&
                  importPath.slice(0, importPath.length - (hashShort + 4)) ===
                    depImportPath
                ) {
                  return `${key}${from}${ql}${depImportPath}.${
                    dep.hash.slice(0, hashShort)
                  }.js${qr}${end || ""}`;
                }
                return s;
            ...
            mod.hash = getHash(mod.jsContent);
            mod.jsFile = `${mod.jsFile.replace(reHashJs, "")}.${
              mod.hash.slice(0, hashShort)
            }.js`;
            Promise.all([
              ensureTextFile(
                mod.jsFile.replace(reHashJs, "") + ".meta.json",
                JSON.stringify(
                  {
                    sourceFile: mod.url,
                    sourceHash: mod.sourceHash,
  ...
  private _resolveImportURL(importer: Module, url: string): string {
    ...
    if (pluginsResolveRet && pluginsResolveRet.external) {
      ...
      if (url in this.config.importMap.imports) {
        url = this.config.importMap.imports[url];
      }
      if (reHttp.test(url)) {
        if (importer.isRemote) {
          rewrittenURL = getRelativePath(
            path.dirname(
              importer.url.replace(reHttp, "/-/").replace(/:(\d+)/, "/$1"),
            ),
            fixImportUrl(url),
          );
        } else {
        ...
        if (importer.isRemote) {
          ...
          if (!pathname.startsWith("/")) {
            pathname = util.cleanPath(
              path.dirname(modUrl.pathname) + "/" + url,
            );
          }
          const importUrl = new URL(
            modUrl.protocol + "//" + modUrl.host + pathname,
          );
          rewrittenURL = getRelativePath(
          ...
          rewrittenURL = url.replace(reModuleExt, "") + "." +
            "x".repeat(hashShort);
        }
      }
    }
    ...
    if (reHttp.test(url)) {
      importer.deps.push(
        { url, hash: "", external: pluginsResolveRet?.external },
      );
    } else {
      if (importer.isRemote) {
        ...
        if (!pathname.startsWith("/")) {
          pathname = util.cleanPath(
            path.dirname(sourceUrl.pathname) + "/" + url,
          );
        }
        importer.deps.push({
  ...
  private async _renderPage(loc: { pathname: string; search?: string }) {
    ...
    try {
      ...
      const { default: App } = appModule
        ? await import("file://" + appModule.jsFile)
        : {} as any;
      const pageComponentTree: { id: string; Component?: any }[] =
        pageModuleTree.map(({ id }) => ({ id }));
      ...
      const {
        ...
        [
          appModule
            ? this._lookupDeps(appModule.id).filter((dep) => !!dep.isStyle)
            : [],
          ...pageModuleTree.map(({ id }) =>
            this._lookupDeps(id).filter((dep) => !!dep.isStyle)
          ).flat(),
        ].flat(),
      );
      ret.head = head;
      ret.scripts = await Promise.all(
        scripts.map(async (script: Record<string, any>) => {
          if (script.innerText && !this.isDev) {
            return {
              ...script,
              innerText: (await minify(script.innerText)).code,
            };
          }
          return script;
        }),
  ...
  private async _render404Page(
    ...
    try {
      ...
      const { default: E404 } = e404Module
        ? await import("file://" + e404Module.jsFile)
        : {} as any;
      const {
        head,
        body,
        ...
        [],
        e404Module
          ? this._lookupDeps(e404Module.id).filter((dep) => !!dep.isStyle)
          : [],
      );
      ret.head = head;
      ret.scripts = await Promise.all(
  ...
  private _lookupDeps(
    ...
    __deps.push(
      ...mod.deps.filter(({ url }) =>
        __deps.findIndex((i) => i.url === url) === -1
      ),
    );
...
export function injectHmr({ id, sourceFilePath, jsContent }: Module): string {
  ...
  const lines = [
    `import { createHotContext, RefreshRuntime, performReactRefresh } from ${
      JSON.stringify(hmrImportPath)
    };`,
    `import.meta.hot = createHotContext(${JSON.stringify(id)});`,
  ];
  const reactRefresh = id.endsWith(".js") || id.endsWith(".md") ||
    id.endsWith(".mdx");
  if (reactRefresh) {
    lines.push("");
    lines.push(
...
function fixImportUrl(importUrl: string): string {
  ...
  if (
    isRemote && !reModuleExt.test(ext) && !reStyleModuleExt.test(ext) &&
    !reMDExt.test(ext)
  ) {
    ext = ".js";
  }
  let pathname = util.trimSuffix(url.pathname, ext);
  let search = Array.from(url.searchParams.entries()).map(([key, value]) =>
    value ? `${key}=${value}` : key
  );
  if (search.length > 0) {
  ...
  return !isRemote && importUrl.startsWith("/api/")
    ? decodeURI(result)
    : result;
}
...
async function cleanupCompilation(jsFile: string) {
  ...
  for await (const entry of Deno.readDir(dir)) {
    if (
      ...
      const _jsName =
        util.trimSuffix(entry.name, ".map").split(".").slice(0, -2).join(".") +
        ".js";
