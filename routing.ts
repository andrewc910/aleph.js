...
export class Routing {
  ...
  update(module: RouteModule, pipeline: string = "web") {
    const newRoute: Route = {
      path: getPagePath(module.id),
      module,
      pipeline,
    };
    ...
    this._lookup((routePath) => {
      ...
      if (
        !newRoute.path.endsWith("/") && path.startsWith(newRoute.path + "/")
      ) {
        ...
        (newRoute.children || (newRoute.children = [])).push(
          { ...route, path: util.trimPrefix(path, newRoute.path) },
        );
  ...
  createRouter(
    ...
    this._lookup((routePath) => {
      const path = routePath.map((r) => r.path).join("");
      const [matchedParams, ok] = matchPath(path, pathname);
      if (ok) {
        tree = routePath.map((r) => r.module);
        // pipeline = routePath[0].pipeline
        const c = routePath[routePath.length - 1].children?.find((c) =>
          c.path === "/"
        );
        if (c) {
          tree.push(c.module);
    ...
    return [{ locale, pathname, pagePath, params, query, pipeline }, tree];
  }
  ...
  // Iterate through all the routes. If we find a route, return false
  private _lookup(
...
export function getPagePath(moduleId: string): string {
  const id = util.trimSuffix(moduleId, ".js").replace(reMDExt, "").toLowerCase()
    .replace(/^\/pages\//, "/").replace(/\/?index$/, "/");
...
function matchPath(
  ...
  for (let i = 0; i < depth; i++) {
    ...
    if (
      ...
      routeSeg.length > 5 && i === routeSegments.length - 1
    ) {
      params[routeSeg.slice(4, -1)] = locSegments.slice(i).map(
        decodeURIComponent,
      ).join("/");
      break;
    }
    ...
    if (
      routeSeg.startsWith("[") && routeSeg.endsWith("]") && routeSeg.length > 2
    ) {
...
export function createPageProps(
  componentTree: { id: string; Component?: ComponentType<any> }[],
): PageProps {
...
function _createPagePropsSegment(
  seg: { id: string; Component?: ComponentType<any> },
): PageProps {
  const pageProps: PageProps = {
    Page: null,
    pageProps: {},
